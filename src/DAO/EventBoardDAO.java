...
public class EventBoardDAO {
	...
	public int insertBoard(EventBoardVo vo) {
		int result = 0;
		String sql = null;
		try {
			//DB접속
			...
			sql = "update EventBoard set b_group = b_group +1";
			pstmt = con.prepareStatement(sql);
			pstmt.executeUpdate();
			
			//insert SQL문 만들기 //b_group , b_level 0 0 으로 insert 규칙3
			sql = "insert into EventBoard (b_idx, b_id, b_pw, b_name, "
							+ "b_email, b_title, b_content, b_group, "
							+ "b_level, b_date, b_cnt) "
							+ " values (EventBoard_b_idx.nextVal, ?,?,?,?,?,?,0,0,sysdate,0)";
	...
	public int getTotalRecord() {
		...
		try {
			...
			pstmt = con.prepareStatement("select count(*) as cnt from EventBoard");
	...
	public int getTotalRecord(String key,String word) {
		...
		if(!word.equals("")) {//검색어를 입력했다면 ?
			...
			if(key.equals("titleContent")) {//검색 기준값 제목+내용을 선택했다면 ?
				...
				sql = "select count(*) as cnt from EventBoard "
						+ " where b_title like '%"+word+"%' "
						+ " or b_content like '%"+word+"%'";
			}else{//검색 기준값 작성자를 선택했다면?
				
				sql = "select count(*) as cnt from EventBoard where b_name like '%"+word+"%'";
			}
	...
	public ArrayList boardListAll() {
		...
		try {
			con = ds.getConnection();
			sql = "select * from EventBoard order by b_group asc";
			pstmt = con.prepareStatement(sql);
	...
	public ArrayList boardList(String key,String word) {
		...
		if(!word.equals("")) {//검색어를 입력했다면 ?
			...
			if(key.equals("titleContent")) {//검색 기준값 제목+내용을 선택했다면 ?
				...
				sql = "select * from EventBoard "
						+ " where b_title like '%"+word+"%' "
						+ " or b_content like '%"+word+"%' order by b_group asc";
			}else{//검색 기준값 작성자를 선택했다면?
				
				sql = "select * from EventBoard where b_name like '%"+word+"%' order by b_group asc";
			}
	...
	public EventBoardVo boardRead(String b_idx) {
		...
		try {
			...
			sql = "UPDATE EventBoard SET b_cnt = b_cnt +1 where b_idx = ?";
			...
			sql = "select * from EventBoard where b_idx = ?";
			...
			if(rs.next()) {
				...
				vo = new EventBoardVo(rs.getInt("b_idx"),
									rs.getString("b_id"),
									rs.getString("b_pw"),
									rs.getString("b_name"), 
									rs.getString("b_email"), 
	...
	public boolean passCheck(String b_idx_, String password) {
		...
		try {
			...
			String sql = "select * from EventBoard where b_pw=? and b_idx=? order by b_idx desc";
	...
	public int updateBoard(String idx_, String email_, String title_, String content_) {
		...
		try {
			...
			String sql = "UPDATE EventBoard SET"
	...
	public String deleteBoard(String delete_idx) {
		...
		try {
			...
			String sql = "DELETE FROM EventBoard WHERE b_idx='"+delete_idx+"'";
	...
	public void replyInsertBoard(String super_b_idx,
		...
		try {
			...
			sql = "select b_group, b_level from EventBoard where b_idx='"+super_b_idx+"'";
			pstmt = con.prepareStatement(sql);
			rs = pstmt.executeQuery();
			rs.next();
			...
			//	 1증가하여 update시켜야 합니다
			sql = "update EventBoard set b_group = b_group + 1 where b_group > '"+b_group+"'";
			pstmt = con.prepareStatement(sql);
			pstmt.executeUpdate();
